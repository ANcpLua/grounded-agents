...
On grounded-agents itself: the type-level design is the interesting part. Grounded<T> only existing when tool-call evidence satisfies the requirement, deny-by-default approvals, closed unions forcing callers to handle the ungrounded case, that's a real idea about agent reliability, and it's more transferable than the retail scenario wrapped around it. A capstone with 1 star isn't a product yet, but the boundary pattern extracted as a standalone library, documented well, published to NuGet under your existing account, is something the .NET agent crowd doesn't currently have a clean answer for. That's the asset. Sponsorship, consulting, or a hosted product can attach to it later, none of which needs Microsoft's permission.On grounded-agents itself: the type-level design is the interesting part. Grounded<T> only existing when tool-call evidence satisfies the requirement, deny-by-default approvals, closed unions forcing callers to handle the ungrounded case, that's a real idea about agent reliability, and it's more transferable than the retail scenario wrapped around it. A capstone with 1 star isn't a product yet, but the boundary pattern extracted as a standalone library, documented well, published to NuGet under your existing account, is something the .NET agent crowd doesn't currently have a clean answer for. That's the asset. Sponsorship, consulting, or a hosted product can attach to it later, none of which needs Microsoft's permission.On grounded-agents itself: the type-level design is the interesting part. Grounded<T> only existing when tool-call evidence satisfies the requirement, deny-by-default approvals, closed unions forcing callers to handle the ungrounded case, that's a real idea about agent reliability, and it's more transferable than the retail scenario wrapped around it. A capstone with 1 star isn't a product yet, but the boundary pattern extracted as a standalone library, documented well, published to NuGet under your existing account, is something the .NET agent crowd doesn't currently have a clean answer for. That's the asset. Sponsorship, consulting, or a hosted product can attach to it later, none of which needs Microsoft's permission.
...
Microsoft Foundry. The repository holds one runnable .NET 10 workflow project and its tests, and
each step of the assignment has a working counterpart here, a Foundry IQ knowledge base, Model
Context Protocol (MCP) tools, a multi-agent workflow with OpenTelemetry tracing, and a fail-closed
evaluation gate.

The submission's design document (German, as the assignment) is [docs/design.md](docs/design.md),
also rendered as [docs/design.pdf](docs/design.pdf).
...
| Project | Contents | GOAL.md step |
| --- | --- | --- |
| [Hosted-FoundryWorkflow](Hosted-FoundryWorkflow/README.md) | A knowledge agent and an MCP tool agent composed into one retail-ops workflow, with a typed handover boundary, OpenTelemetry tracing and an evaluation gate | Steps 1-3: multi-agent design, observability, evaluation, end-to-end workflow |
...
The workflow requires a Foundry agent with a Foundry IQ knowledge base.
[docs/foundry-iq-setup.md](docs/foundry-iq-setup.md) creates both in the Microsoft Foundry portal.
...
- An Azure subscription with a Microsoft Foundry project, a deployed chat model, and the
  `product-expert-agent` with its Foundry IQ knowledge base, set up with
  [docs/foundry-iq-setup.md](docs/foundry-iq-setup.md)
...
├── docs/                          Design document (Markdown source, PDF, diagrams), Foundry IQ setup
├── Hosted-FoundryWorkflow/        The multi-agent workflow
...
The workflow treats the Foundry boundary as a type-design problem: the failure modes of an
...
Rules live in the root `.markdownlint.jsonc`. `GOAL.md` keeps its upstream formatting, and the
documents in `docs/` follow pandoc and portal-guide conventions; both are excluded:

```bash
npx markdownlint-cli2 "**/*.md" "#GOAL.md" "#docs"
```
